...
save_path = "./tmp_ip_adpater_result_select"
if not os.path.exists(save_path):
    os.makedirs(save_path)

style_path = "./tmp_example_img/style_imgs"
wordart_image_path = "./tmp_example_img/wordart_yellow"

# for i,it in enumerate(list(a.keys())):
# ...     print(i,a[it])

# prompt related
negative_prompt = "obese, deformed, distorted, disfigured, poorly drawn, bad anatomy, wrong anatomy, low quality, long neck, frame, text, worst quality,watermark, deformed, ugly,  blur, out of focus,"
...
prompt_dict = {
    ...
    # "shanshui_white":"landscape scenery, mountain, waterfall, clouds, trees, lake, artistic in dark",
    ...
    "dunhuanggirl_yellow":"Dunhuang style dancing female, Chinese tradition peri girl in yellow golden color, dark background, detail face",
}
# read image prompt
    ...
    for sub_style_folder, style_prompt in prompt_dict.items():
        ...
        for style_img in style_image_list:
            ...
            for i, it in enumerate(images):
                save_folder = os.path.join(save_path,sub_style_folder+"_"+style_n)
                if not os.path.exists(save_folder):
                    os.makedirs(save_folder)
                save_name = os.path.join(save_folder, "%s_%d.jpg" % (word_n, i))
                it.save(save_name)
