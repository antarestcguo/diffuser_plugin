...
from plugin_modules.ChosenOne.sdxl_the_chosen_one import train as train_pipeline
import shutil
    ...
    for loop in range(start_from, loop_num):
        ...
        if loop == 0:
            ...
            pipe = load_trained_pipeline(model_path=args.output_dir_per_loop,
                                         load_lora=True,
                                         lora_path=os.path.join(args.output_dir_per_loop,
        ...
        if loop == 0:
            ...
            if pairwise_distances < init_dist * args.convergence_scale:
                ...
                print("###########################################################################")
                print(
                    f"Converge at {loop}. Target distance: {init_dist}, current pairwise distance: {pairwise_distances}. Final model saved at {os.path.join(output_dir_base, args.character_name, str(loop - 1))}")
                print("###########################################################################")
                print("###########################################################################")
                print()
        ...
        # evaluate
        center_norms = np.linalg.norm(centers[labels] - elements, axis=-1,
                                      keepdims=True)  # each data point subtract its coresponding center
...
def load_dinov2():
    dinov2_vitl14 = torch.hub.load('./models/dinov2', 'dinov2_vitl14',source='local',trust_repo=True).cuda()
    dinov2_vitl14.eval()
    return dinov2_vitl14


if __name__ == "__main__":
    args = config_2_args("./plugin_modules/ChosenOne/config/theChosenOne_bluehat.yaml")
