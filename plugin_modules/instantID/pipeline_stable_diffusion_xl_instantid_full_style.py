...
from plugin_modules.instantID.ip_adapter.resampler import Resampler
from plugin_modules.instantID.ip_adapter.utils import is_torch2_available

if is_torch2_available():
    from plugin_modules.instantID.ip_adapter.attention_processor_instantstyle import \
        IPAttnProcessor2_0 as IPAttnProcessor, AttnProcessor2_0 as AttnProcessor, \
        IPStyleAttnProcessor2_0 as IPStyleProcessor
else:
    from plugin_modules.instantID.ip_adapter.attention_processor_instantstyle import IPAttnProcessor, AttnProcessor
from plugin_modules.instantID.ip_adapter.attention_processor_instantstyle import region_control
...
class StableDiffusionXLInstantIDPipeline(StableDiffusionXLControlNetPipeline):
    ...
    def init_proj(self):
        image_proj_model = ImageProjModel(
            ...
            clip_extra_context_tokens=self.instant_style_num_tokens,
        ).to(self.device, dtype=torch.float16)
        return image_proj_model

    def load_ip_adapter_instantid(self, model_ckpt, ip_adapter_ckpt, image_encoder_path, image_emb_dim=512,
                                  instant_id_num_tokens=16, instant_style_num_tokens=4, id_scale=0.5, style_scale=1.0,
                                  target_blocks=["block"]):
        # init param
        self.instant_id_num_tokens = instant_id_num_tokens
        self.instant_style_num_tokens = instant_style_num_tokens
        self.image_encoder_path = image_encoder_path

        # load instantid and instantstyle ipadapter
        self.set_image_proj_model(model_ckpt, image_emb_dim, instant_id_num_tokens)
        self.set_ip_adapter(self.instant_id_num_tokens, self.instant_style_num_tokens, id_scale, style_scale,
                            target_blocks)

        # load origin image_encoder
        self.image_encoder = CLIPVisionModelWithProjection.from_pretrained(image_encoder_path).to(
            self.device, dtype=torch.float16
        )
        self.clip_image_processor = CLIPImageProcessor()
        # image proj model
        self.style_image_proj_model = self.init_proj()

        # load both instantid and instantstyle
        self.load_ip_adapter(model_ckpt, ip_adapter_ckpt)
    ...
    def load_ip_adapter(self, instant_id_model_ckpt, instant_style_model_ckpt):
        # load instantID
        instant_id_state_dict = torch.load(instant_id_model_ckpt, map_location="cpu")
        if 'ip_adapter' in instant_id_state_dict:
            instant_id_state_dict = instant_id_state_dict['ip_adapter']

        # load instantstyle and init image_proj_model
        if os.path.splitext(instant_style_model_ckpt)[-1] == ".safetensors":
            instant_style_state_dict = {"image_proj": {}, "ip_adapter": {}}
            with safe_open(instant_style_model_ckpt, framework="pt", device="cpu") as f:
                for key in f.keys():
                    if key.startswith("image_proj."):
                        instant_style_state_dict["image_proj"][key.replace("image_proj.", "")] = f.get_tensor(key)
                    elif key.startswith("ip_adapter."):
                        instant_style_state_dict["ip_adapter"][key.replace("ip_adapter.", "")] = f.get_tensor(key)
        else:
            instant_style_state_dict = torch.load(instant_style_model_ckpt, map_location="cpu")
        self.style_image_proj_model.load_state_dict(instant_style_state_dict["image_proj"])

        total_state_dict = instant_id_state_dict.copy()
        # split instantid and instantstyle
        # for name, ip_key in zip(self.unet.attn_processors.keys(), instant_id_state_dict.keys()):
        #     if not hasattr(self.unet.attn_processors[name], "style"):
        #         continue
        #     if self.unet.attn_processors[name].style:
        #         print("style", name)
        #         print(ip_key, total_state_dict[ip_key].shape, '->', ip_key,
        #               instant_style_state_dict["ip_adapter"][ip_key].shape)
        #         total_state_dict[ip_key] = instant_style_state_dict["ip_adapter"][ip_key]
        for name, ip_key in zip(self.unet.attn_processors.keys(), instant_id_state_dict.keys()):
            if not hasattr(self.unet.attn_processors[name], "style"):
                continue  # 无ip-adpater的无self.to_k_ip,to_v_ip
            else:  # style 和ip 都load，根据是否选择style在前传的时候确定叠加那个hidden
                print("style", name)
                # modify ip_key
                style_key = ip_key.replace("ip", "style")
                total_state_dict[style_key] = instant_style_state_dict["ip_adapter"][ip_key]

        # load
        ip_layers = torch.nn.ModuleList(self.unet.attn_processors.values())
        ip_layers.load_state_dict(total_state_dict, strict=False)

    def set_ip_adapter(self, instant_id_num_tokens, instant_style_num_tokens, id_scale, style_scale, target_blocks=[]):
        unet = self.unet
        attn_procs = {}
        for name in unet.attn_processors.keys():
            ...
            if cross_attention_dim is None:
                ...
                #         id_num_tokens=instant_id_num_tokens,
                #         style_num_tokens=instant_style_num_tokens,
                #         style=True,
                ...
                #         style=False
                #     ).to(unet.device, dtype=unet.dtype)
                if style:  # modify , IPStyleProcessor, including self.to_k_ip,to_k_style
                    attn_procs[name] = IPStyleProcessor(
                        hidden_size=hidden_size,
                        cross_attention_dim=cross_attention_dim,
                        id_scale=id_scale,
                        style_scale=style_scale,
    ...
    def set_ip_adapter_scale(self, id_scale, style_scale):
        unet = getattr(self, self.unet_name) if not hasattr(self, "unet") else self.unet
        for attn_processor in unet.attn_processors.values():
            if isinstance(attn_processor, IPAttnProcessor):
                if attn_processor.style:
                    attn_processor.style_scale = style_scale
                else:
                    attn_processor.id_scale = id_scale
    ...
    def get_style_image_embeds(self, pil_image=None, clip_image_embeds=None, content_prompt_embeds=None):
    ...
    def __call__(
        ...
        if ip_adapter_scale is not None and style_adapter_scale:
            self.set_ip_adapter_scale(ip_adapter_scale, style_adapter_scale)
            ...
            if neg_content_prompt is not None:
                with torch.inference_mode():
                    (
                        ...
                        neg_content_prompt,
                        num_images_per_prompt=1,  # num_samples,
        ...
        image_prompt_embeds, uncond_image_prompt_embeds = self.get_style_image_embeds(style_image,
