...
def rescale_noise_cfg(noise_cfg, noise_pred_text, guidance_rescale=0.0):
    ...
    return noise_cfg
class StableDiffusionXLPipeline(StableDiffusionXLPipeline):
    @torch.no_grad()
    def __call__(
            self,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if image is not None:
                    zt = self.prepare_img_latents(z0, t.repeat(1), 1, num_images_per_prompt, prompt_embeds.dtype,
                                                  device, generator, True)  # add_noise/
