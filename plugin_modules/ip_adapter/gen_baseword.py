...
def b_text_can_be_draw(font_path, text_to_draw):
    ...
    # import pdb
    # pdb.set_trace()
    # if ord(text_to_draw) in uniMap:
    #     glyfMapDict = font['glyf']
    #     contour_len = len(glyfMapDict[ord(text_to_draw)].getCoordinates(0)[0])
    #     if contour_len != 0:
    #         b_draw = True
    # return b_draw
...
def create_maxtext_image_RGB_oldversion(text, bg_color, fg_color, font_path, font_size, img_h=1024, img_w=1024, text_ratio=0.85):
    # bg image
    image_bg = Image.new("RGB", (img_w, img_h), bg_color)  # RGB, not RGBA
...
def create_maxtext_image_RGB(text, bg_color, fg_color, font_path, font_size, img_h=1024, img_w=1024,
                                text_ratio=0.85):
    text_to_draw = text[0]
    select_font_path = font_path
    # check if can be draw
    if b_text_can_be_draw(font_path, text_to_draw):
        font = ImageFont.truetype(font_path, font_size)
    else:
        b_draw = False
        for it_default_font_path in default_font_path:
            b_draw = b_text_can_be_draw(it_default_font_path, text_to_draw)
            if b_draw:
                font = ImageFont.truetype(it_default_font_path, font_size)
    ...
    # img text
    image_text = Image.new("RGB", (img_w, img_h), bg_color)  # RGB, not RGBA
    draw = ImageDraw.Draw(image_text)
